...
################################################################################
# Export: biology gene table for both Human and Mouse KD
# Mirrors the format of UBR5_biology_gene_table_ALL.csv:
#   Class | Gene | baseMean | log2FoldChange | lfcSE | stat | pvalue | padj
#
# Source: unshrunken Wald results ($deseq$res) — preserves the stat column
# used for GSEA ranking and matching the reference table's source.
# Genes absent from DE results (filtered or not fitted) appear as NA rows
# so each output always has the same 86 rows as the reference panel.
#
# Run AFTER both pipeline calls:
#   human_KD_results <- run_kd_pipeline(analysis_configs$human_KD_vs_Control)
#   mouse_KD_results <- run_kd_pipeline(analysis_configs$mouse_KD_vs_Control)
################################################################################

# ── 1. Panel definition ────────────────────────────────────────────────────────
# Mouse symbols (title-case) are the reference; human symbols are uppercase
# with two orthologs that differ beyond simple case:
#   Icosl  (mouse) -> ICOSLG (human)
#   Oasl1  (mouse) -> OASL   (human)

bio_gene_panel <- tribble(
  ~mouse_gene, ~human_gene, ~Class,
  # Chromatin / SAGA / retroelement control
  "Trrap",    "TRRAP",    "Chromatin / SAGA / retroelement control",
  "Kat2a",    "KAT2A",    "Chromatin / SAGA / retroelement control",
  "Tada3",    "TADA3",    "Chromatin / SAGA / retroelement control",
  "Taf5l",    "TAF5L",    "Chromatin / SAGA / retroelement control",
  "Taf6",     "TAF6",     "Chromatin / SAGA / retroelement control",
  "Taf9",     "TAF9",     "Chromatin / SAGA / retroelement control",
  "Dnmt1",    "DNMT1",    "Chromatin / SAGA / retroelement control",
  "Hdac1",    "HDAC1",    "Chromatin / SAGA / retroelement control",
  # MYC-associated biosynthetic machinery
  "Myc",      "MYC",      "MYC-associated biosynthetic machinery",
  "Mycn",     "MYCN",     "MYC-associated biosynthetic machinery",
  "Eif3b",    "EIF3B",    "MYC-associated biosynthetic machinery",
  "Eif3c",    "EIF3C",    "MYC-associated biosynthetic machinery",
  "Eif3d",    "EIF3D",    "MYC-associated biosynthetic machinery",
  "Eif3e",    "EIF3E",    "MYC-associated biosynthetic machinery",
  "Eif3f",    "EIF3F",    "MYC-associated biosynthetic machinery",
  "Eif3g",    "EIF3G",    "MYC-associated biosynthetic machinery",
  "Eif3h",    "EIF3H",    "MYC-associated biosynthetic machinery",
  "Eif3i",    "EIF3I",    "MYC-associated biosynthetic machinery",
  "Eif3j1",   "EIF3J1",   "MYC-associated biosynthetic machinery",
  "Eif3k",    "EIF3K",    "MYC-associated biosynthetic machinery",
  "Eif3l",    "EIF3L",    "MYC-associated biosynthetic machinery",
  "Eif3m",    "EIF3M",    "MYC-associated biosynthetic machinery",
  "Eif4g1",   "EIF4G1",   "MYC-associated biosynthetic machinery",
  "Eif4g2",   "EIF4G2",   "MYC-associated biosynthetic machinery",
  "Abce1",    "ABCE1",    "MYC-associated biosynthetic machinery",
  "Hnrnpa1",  "HNRNPA1",  "MYC-associated biosynthetic machinery",
  "Gnl3",     "GNL3",     "MYC-associated biosynthetic machinery",
  "Kpnb1",    "KPNB1",    "MYC-associated biosynthetic machinery",
  # Pro-tumor myeloid / TAM recruitment
  "Ccl2",     "CCL2",     "Pro-tumor myeloid / TAM recruitment",
  "Csf1",     "CSF1",     "Pro-tumor myeloid / TAM recruitment",
  "Cx3cl1",   "CX3CL1",   "Pro-tumor myeloid / TAM recruitment",
  "Ccl7",     "CCL7",     "Pro-tumor myeloid / TAM recruitment",
  "Wnt5a",    "WNT5A",    "Pro-tumor myeloid / TAM recruitment",
  "Tgfb3",    "TGFB3",    "Pro-tumor myeloid / TAM recruitment",
  "Pdgfc",    "PDGFC",    "Pro-tumor myeloid / TAM recruitment",
  "Stat3",    "STAT3",    "Pro-tumor myeloid / TAM recruitment",
  "Ctnnb1",   "CTNNB1",   "Pro-tumor myeloid / TAM recruitment",
  "Vegfa",    "VEGFA",    "Pro-tumor myeloid / TAM recruitment",
  "Vegfb",    "VEGFB",    "Pro-tumor myeloid / TAM recruitment",
  "Icosl",    "ICOSLG",   "Pro-tumor myeloid / TAM recruitment",   # human ortholog differs
  # RNA homeostasis / decay
  "Cnot1",    "CNOT1",    "RNA homeostasis / decay",
  "Cnot2",    "CNOT2",    "RNA homeostasis / decay",
  "Cnot3",    "CNOT3",    "RNA homeostasis / decay",
  "Cnot7",    "CNOT7",    "RNA homeostasis / decay",
  "Cnot8",    "CNOT8",    "RNA homeostasis / decay",
  "Cnot9",    "CNOT9",    "RNA homeostasis / decay",
  "Cnot10",   "CNOT10",   "RNA homeostasis / decay",
  "Cnot11",   "CNOT11",   "RNA homeostasis / decay",
  "Ythdf1",   "YTHDF1",   "RNA homeostasis / decay",
  "Ythdf2",   "YTHDF2",   "RNA homeostasis / decay",
  "Ddx1",     "DDX1",     "RNA homeostasis / decay",
  "Nono",     "NONO",     "RNA homeostasis / decay",
  "Sfpq",     "SFPQ",     "RNA homeostasis / decay",
  # Tissue remodeling / EMT / stromal axis
  "Timp1",    "TIMP1",    "Tissue remodeling / EMT / stromal axis",
  "Has2",     "HAS2",     "Tissue remodeling / EMT / stromal axis",
  "Itgb8",    "ITGB8",    "Tissue remodeling / EMT / stromal axis",
  "Emp3",     "EMP3",     "Tissue remodeling / EMT / stromal axis",
  "Tnfaip6",  "TNFAIP6",  "Tissue remodeling / EMT / stromal axis",
  "Ptgir",    "PTGIR",    "Tissue remodeling / EMT / stromal axis",
  "P2rx7",    "P2RX7",    "Tissue remodeling / EMT / stromal axis",
  "Slc7a2",   "SLC7A2",   "Tissue remodeling / EMT / stromal axis",
  "Col1a1",   "COL1A1",   "Tissue remodeling / EMT / stromal axis",
  "Col1a2",   "COL1A2",   "Tissue remodeling / EMT / stromal axis",
  "Col6a2",   "COL6A2",   "Tissue remodeling / EMT / stromal axis",
  "Col6a3",   "COL6A3",   "Tissue remodeling / EMT / stromal axis",
  "Bgn",      "BGN",      "Tissue remodeling / EMT / stromal axis",
  "Postn",    "POSTN",    "Tissue remodeling / EMT / stromal axis",
  "Cdh11",    "CDH11",    "Tissue remodeling / EMT / stromal axis",
  "Pdgfrb",   "PDGFRB",   "Tissue remodeling / EMT / stromal axis",
  "Spp1",     "SPP1",     "Tissue remodeling / EMT / stromal axis",
  # Viral mimicry / dsRNA sensing / ISG
  "Ifih1",    "IFIH1",    "Viral mimicry / dsRNA sensing / ISG",
  "Eif2ak2",  "EIF2AK2",  "Viral mimicry / dsRNA sensing / ISG",
  "Ddx60",    "DDX60",    "Viral mimicry / dsRNA sensing / ISG",
  "Oasl1",    "OASL",     "Viral mimicry / dsRNA sensing / ISG",   # human ortholog differs
  "Oas2",     "OAS2",     "Viral mimicry / dsRNA sensing / ISG",
  "Ifit2",    "IFIT2",    "Viral mimicry / dsRNA sensing / ISG",
  "Cd274",    "CD274",    "Viral mimicry / dsRNA sensing / ISG",
  "Cxcl10",   "CXCL10",   "Viral mimicry / dsRNA sensing / ISG",
  "Samd9l",   "SAMD9L",   "Viral mimicry / dsRNA sensing / ISG",
  "Trim14",   "TRIM14",   "Viral mimicry / dsRNA sensing / ISG",
  "Trim21",   "TRIM21",   "Viral mimicry / dsRNA sensing / ISG",
  "Trim26",   "TRIM26",   "Viral mimicry / dsRNA sensing / ISG",
  "Parp12",   "PARP12",   "Viral mimicry / dsRNA sensing / ISG",
  "Ube2l6",   "UBE2L6",   "Viral mimicry / dsRNA sensing / ISG",
  "Herc6",    "HERC6",    "Viral mimicry / dsRNA sensing / ISG",
  "Il15",     "IL15",     "Viral mimicry / dsRNA sensing / ISG"
)

# ── 2. Export function ─────────────────────────────────────────────────────────
# Joins the curated panel against DE results for one species, reports missing
# genes, and writes the CSV. The Gene column in the output uses the symbol
# as it appears in the DE results (species-appropriate capitalisation), while
# Class order and within-class gene order match the reference panel exactly.

export_biology_gene_table <- function(kd_results_obj,
                                      gene_col,        # "mouse_gene" or "human_gene"
                                      contrast_dir,    # subfolder name under results/deseq2/
                                      out_filename,
                                      species_label) {
  
  message("\n── Biology gene table: ", species_label, " ──")
  
  # Select the relevant symbol column and rename to Gene for the output
  panel <- bio_gene_panel %>%
    select(Gene = all_of(gene_col), Class) %>%
    mutate(
      Class = factor(Class, levels = unique(bio_gene_panel$Class)),
      Gene  = factor(Gene,  levels = unique(Gene))   # preserves panel order
    )
  
  # Pull unshrunken Wald results; keep only columns needed for the output table
  de_res <- kd_results_obj$deseq$res %>%
    select(external_gene_name, baseMean, log2FoldChange, lfcSE, stat, pvalue, padj)
  
  # Left-join: every panel gene gets a row; absent genes get NA statistics
  out_tbl <- panel %>%
    left_join(de_res, by = c("Gene" = "external_gene_name")) %>%
    arrange(Class, Gene) %>%
    mutate(Class = as.character(Class),
           Gene  = as.character(Gene))
  
  # Diagnostic: report genes not found in DE results
  missing <- out_tbl %>% filter(is.na(baseMean)) %>% pull(Gene)
  if (length(missing) > 0) {
    message(sprintf(
      "  NOTE: %d panel gene(s) absent from %s results (filtered or not fitted):\n    %s",
      length(missing), species_label, paste(missing, collapse = ", ")
    ))
  }
  
  out_path <- file.path(output_root, "results", "deseq2",
                        contrast_dir, out_filename)
  write.csv(out_tbl, out_path, row.names = FALSE)
  
  message(sprintf("  Written: %s", out_path))
  message(sprintf("  %d rows | %d with statistics | %d NA",
                  nrow(out_tbl),
                  sum(!is.na(out_tbl$baseMean)),
                  sum(is.na(out_tbl$baseMean))))
  
  invisible(out_tbl)
}

# ── 3. Run for both species ────────────────────────────────────────────────────

export_biology_gene_table(
  kd_results_obj = mouse_KD_results,
  gene_col       = "mouse_gene",
  contrast_dir   = "mouse_KD_vs_Control",
  out_filename   = "mouse_KD_biology_gene_table.csv",
  species_label  = "Mouse KD"
)

export_biology_gene_table(
  kd_results_obj = human_KD_results,
  gene_col       = "human_gene",
  contrast_dir   = "human_KD_vs_Control",
  out_filename   = "human_KD_biology_gene_table.csv",
  species_label  = "Human KD"
)
